...
function execSignalAssign(ctx, ast) {
    ...
    // process ast.values[0] <-- ast.values[1] 

    // process a destination, that must be an pre-existing unassigned SIGNAL ----------------------
    let vDest;
    if (ast.values[0].type == "DECLARE") {
