#[cfg(test)]
mod test {
    use super::super::eval::{Mode,Evaluator};
    ...
    fn doeval(s : &str) -> Result<(Evaluator,Scope)> {
        let mut evaluator = Evaluator::new(Mode::GenConstraints);
    ...
    fn test_bodyp_vars() -> Result<()> {
        let (_,scope) = doeval("var i = 1;var j=5; var k=j;")?; 
    ...
    fn test_infix_vars() -> Result<()> {
        // + * -
        let (_,scope) = doeval("var i = 1+2*3; var j=i-3;")?;
        scope_eq(&scope,"i","Some(Algebra(7))");
        scope_eq(&scope,"j","Some(Algebra(4))");

        // bool & fs == !=
        let (_,scope) = doeval("var iyes=1==1;var ino=1!=1; var byes=iyes==iyes; var bno=iyes!=iyes;")?;  
        ...
        let (_,scope) = doeval("var yes1=1<2;var no1=1>2; var yes2=1<=2; var no2=1>=2;")?; 
    ...
    fn test_prefix_vars() -> Result<()> {        
        let (_,scope) = doeval("var i = -5; var j=-i;")?; 
    ...
    fn test_function() -> Result<()> {
        let (_,scope) = doeval("function f(a) { return a; }\nvar k=f(1);")?; 
        scope_eq(&scope,"k","Some(Algebra(1))");

        let (_,scope) = doeval("function f(a,b) { return a+b; }\nvar k=f(1,2);")?;  
    ...
    fn test_assig_vars()-> Result<()> {
        let (_,scope) = doeval("function f(a) { var t=5; t+=a; t-=2; t*=2; return t; }\nvar k=f(2);")?; 
    ...
    #[test]
    fn test_for() -> Result<()> {        
        let (_,scope) = doeval("function fact(N) { var f=1; for (var i=1;i<=N;i+=1) { f = f * i; } return f;}\nvar out=fact(10);")?; 
    ...
    fn test_for_inner_return() -> Result<()> {
        let (_,scope) = doeval("function fact(N) { var f=1; for (var i=1;i<=N;i+=1) { return N; f = f * i; } return f;}\nvar out=fact(10);")?; 
    ...
    fn test_while() -> Result<()> {
        let (_,scope) = doeval("function fact(N) { var f=1; var i=1; while (i<=N) { f = f * i; i+=1; } return f;}\nvar out=fact(10);")?; 
    ...
    fn test_while_inner_return() -> Result<()> {
        let (_,scope) = doeval("function fact(N) { var f=1; var i=1; while (i<=N) { return N; f = f * i; i+=1; } return f;}\nvar out=fact(10);")?; 
    ...
    #[test]
    fn test_if() -> Result<()> {
        let (_,scope) = doeval("function test(v) { if (v==1) { return 1; } return 2;}\nvar out1=test(1); var out2=test(2);")?; 
    ...
    fn test_if_else() -> Result<()> {
        let (_,scope) = doeval("function test(v) { if (v==1) { return 1; } else { return 2;}}\nvar out1=test(1); var out2=test(2);")?; 
    ...
    fn test_template_signal_base() -> Result<()> {
        let (eval,_) = doeval("template t() { signal a; signal input b; signal private input c; signal output d; }\ncomponent main=t();")?; 
    ...
    fn test_template_first_constrain() -> Result<()> {
        let (eval,_) = doeval("template t() { signal input a; signal input b; signal private input c; c === 5 * a * b  + 5;}\ncomponent main=t();")?; 
   ...
   #[test]
    fn test_signal_fs_assign()-> Result<()> {
        let (eval,_) = doeval("template t() { signal in; signal const; const <-- 2;  2 === 1 + in * const ;}\ncomponent main=t();")?; 
   ...
   #[test]
    fn test_signal_equivalence_constrain() -> Result<()> {
        let (eval,_) = doeval("template t() { signal in; signal out; out <== in; out === 1; }\ncomponent main=t();")?; 
   ...
   #[test]
    ...
    fn test_signal_single_array_assig()  -> Result<()>{
        let (eval,_) = doeval("template t() { signal in[2][2]; for (var i=0;i<2;i+=1) { in[i][0] <-- i+2 ; in[i][1] <--i+3 ; }}\ncomponent main=t();")?; 
    ...
    fn test_signal_single_array_constrain()-> Result<()> {
        let (eval,_) = doeval("template t() { signal in[2][2]; signal s; in[1][0] + in[0][1] === 0 ; }\ncomponent main=t();")?; 
    ...
    fn test_signal_single_array_assig_constrain()-> Result<()> {
        let (eval,_) = doeval("template t() { signal in[2]; signal s; in[0] <== 1 ; in[0] === in[1]; }\ncomponent main=t();")?; 
    ...
    fn test_subcomponent() -> Result<()> {
        let (eval,_) = doeval("template t0() { signal t0in; t0in === 5; } template t1() { signal t1in; component T0 = t0(); t1in <== T0.t0in; }\ncomponent main=t1();")?; 
    ...
    fn test_component_array() -> Result<()> {
        let (eval,_) = doeval("template t0() { signal t0in; t0in === 5; } template t1() { signal t1in; component T0[1]; for (var k=0;k<1;k +=1) { T0[k] = t0(); t1in <== T0[k].t0in; }}\ncomponent main=t1();")?; 
    ...
    fn test_variable_array() -> Result<()> {
        let (_,scope) = doeval("function f() { var k[1]; k[0]=6; return k[0]; }\nvar out=f();")?; 
    ...
    fn test_variable_array_fe_init()-> Result<()> {
        let (_,scope) = doeval("var P=[1,2,3,4,5];\nvar out=P[2];")?; 
